...
# oxygen system
execute if predicate expansion:nbt_checks/selected_item/items/oxygen_tank run return run execute if predicate expansion:armor/any if items entity @s armor.* *[custom_data~{ModStorage:[{components:{"minecraft:custom_data":{oxygen_tank:1b}}}]}] run function expansion:items/oxygen_tank/init
# fleet finder
