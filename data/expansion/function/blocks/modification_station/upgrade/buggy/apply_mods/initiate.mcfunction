...
execute if items block ~ ~ ~ container.0 *[custom_data~{speed_mod:1b}] unless data storage expansion:temp ModStorage.speed_mod.id run data modify entity @s item.components."minecraft:custom_data".ModStorage.speed_mod set from block ~ ~ ~ Items[{Slot:0b}]
execute if items block ~ ~ ~ container.9 *[custom_data~{oxygen_tank:1b}] unless data storage expansion:temp ModStorage.oxygen_tank.id run data modify entity @s item.components."minecraft:custom_data".ModStorage.oxygen_tank set from block ~ ~ ~ Items[{Slot:9b}]
