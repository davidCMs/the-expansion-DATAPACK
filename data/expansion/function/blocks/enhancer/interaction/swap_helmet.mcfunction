...
execute on vehicle if score #temp exp.bool matches 1 unless items entity @s contents * if items entity @p[tag=exp.clicked] armor.head *[minecraft:custom_data~{exp.enhanceable:1b}] at @s run return run function expansion:blocks/enhancer/interaction/insert_helmet
