...
data merge storage expansion:temp {cold_resist:[[{text:"Cold Resistance: ",color:"gold"}],[{text:"",color:"green"},{score:{name:"#temp",objective:"exp.cold_resist"}}]]}
execute if score #temp exp.cold_resist matches ..-1 run data modify storage expansion:temp cold_resist[1][0].color set value "red"
