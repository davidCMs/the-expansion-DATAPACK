...
data merge storage expansion:temp {heat_resist:[[{text:"Heat Resistance: ",color:"aqua"}],[{text:"",color:"green"},{score:{name:"#temp",objective:"exp.heat_resist"}}]]}
execute if score #temp exp.heat_resist matches ..-1 run data modify storage expansion:temp heat_resist[1][0].color set value "red"
