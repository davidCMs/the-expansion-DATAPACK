...
execute as @e[type=#expansion:turret_target,tag=!exp.zap_target,tag=!exp.ignore_for_damage,distance=1.1..10,limit=1,sort=random] run return run data modify storage expansion:owner data.current_UUID set from entity @s UUID
