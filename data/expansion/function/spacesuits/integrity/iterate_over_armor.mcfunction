...
# default values for durability damage and max
scoreboard players reset #temp exp.health
scoreboard players reset #temp exp.max_health

# get the equipments max durability score (putting this here makes the percentage lag one iteration behind on the actionbar but I can't be bothered with that now)
...
execute if score #temp exp.damage matches 1.. run function expansion:spacesuits/temperature_damage/apply
