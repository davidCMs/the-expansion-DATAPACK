...
execute if entity @s[tag=exp.rocket_pilot] as @n[type=minecraft:armor_stand,tag=exp.rocket,distance=..7] on passengers if entity @s[type=interaction] run tag @s add exp.rocket_pilot_rcdet
