...
execute if score @s exp.max_range matches 0 summon marker run function expansion:vehicles/mech/actions/righthand/rocket/target/lock

# lock a target if a suitable target is found
execute as @e[type=!#expansion:ignore,tag=!exp.block,tag=!exp.vehicle,tag=!exp.ignore,distance=..5,limit=1,sort=nearest] run return run function expansion:vehicles/mech/actions/righthand/rocket/target/lock
